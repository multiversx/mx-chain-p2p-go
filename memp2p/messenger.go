...
// Messenger is an implementation of the common.Messenger interface that
...
type Messenger struct {
	...
	topicValidators map[string]common.MessageProcessor
	topicsMutex     *sync.RWMutex
	seqNo           uint64
	processQueue    chan common.MessageP2P
	numReceived     uint64
}
...
func NewMessenger(network *Network) (*Messenger, error) {
	...
	messenger := &Messenger{
		...
		topicValidators: make(map[string]common.MessageProcessor),
		topicsMutex:     &sync.RWMutex{},
		processQueue:    make(chan common.MessageP2P, maxQueueSize),
	}
...
func (messenger *Messenger) CreateTopic(name string, _ bool) error {
	...
	if found {
		return common.ErrTopicAlreadyExists
	}
...
func (messenger *Messenger) RegisterMessageProcessor(topic string, _ string, handler common.MessageProcessor) error {
	if check.IfNil(handler) {
		return common.ErrNilValidator
	}
	...
	if !found {
		return fmt.Errorf("%w RegisterMessageProcessor, topic: %s", common.ErrNilTopic, topic)
	}

	validator := messenger.topicValidators[topic]
	if !check.IfNil(validator) {
		return common.ErrTopicValidatorOperationNotSupported
	}
...
func (messenger *Messenger) UnregisterMessageProcessor(topic string, _ string) error {
	...
	_, found := messenger.topics[topic]
	if !found {
		return fmt.Errorf("%w UnregisterMessageProcessor, topic: %s", common.ErrNilTopic, topic)
	}
...
func (messenger *Messenger) OutgoingChannelLoadBalancer() common.ChannelLoadBalancer {
	return nil
}
...
func (messenger *Messenger) receiveMessage(message common.MessageP2P) {
...
func (messenger *Messenger) SetPeerShardResolver(_ common.PeerShardResolver) error {
	return nil
}

// SetPeerDenialEvaluator does nothing
func (messenger *Messenger) SetPeerDenialEvaluator(_ common.PeerDenialEvaluator) error {
	return nil
}

// GetConnectedPeersInfo returns a nil object. Not implemented.
func (messenger *Messenger) GetConnectedPeersInfo() *common.ConnectedPeersInfo {
	return nil
}
